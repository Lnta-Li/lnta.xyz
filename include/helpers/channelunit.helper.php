...
{
    function GetTopField($tid, $field)
    {
        global $dsql;
        $topid = GetTopid($tid);
 ...
 * @return    string
 */
if ( ! function_exists('GetTypeField'))
{
    function GetTypeField($tid, $field)
    {
        global $dsql;
        $row = $dsql->GetOne("SELECT * FROM `#@__arctype` WHERE `id`= {$tid}");
        if($row[$field]=="")
        {
            $fatherid = GetTopid($tid);
            $rowfather = $dsql->GetOne("SELECT * FROM `#@__arctype` WHERE `id`= {$fatherid}");
            if($rowfather[$field]=="")
            {
                return "该字段为空，请到系统管理后台填充该字段";
            }
            else
            {
                return $rowfather[$field];
            }
        }
        else
        {
