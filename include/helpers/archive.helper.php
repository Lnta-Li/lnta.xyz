    ...
    function UpIndexKey($id, $arcrank, $typeid, $sortrank=0, $tags='')
    {
        ...
        */
        if($tags!='')
        {
            ...
            {
                $tag = trim($tag);
                if(isset($tag[250]) || $tag!=stripslashes($tag))
                {
                    continue;
                }
...
if ( ! function_exists('InsertOneTag'))
{
    function InsertOneTag($tag, $aid)
    {
        ...
        if(!is_array($row))
        {
            $rs = $dsql->ExecuteNoneQuery(" INSERT INTO `#@__tagindex`(`tag`,`tag_alias`,`typeid`,`count`,`total`,`weekcc`,`monthcc`,`weekup`,`monthup`,`addtime`) VALUES('$tag','$tag','$typeid','0','1','0','0','$addtime','$addtime','$addtime'); ");
            $tid = $dsql->GetLastID();
        }
        else
        {
