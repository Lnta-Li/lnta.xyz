 ...
 * 获取栏目列表标签
 *
 * @version        $Id: channel.lib.php 1 9:29 2010年7月6日 $
    ...
    for($i=0;$i < $line;$i++)
    {
        ...
        for($j=0; $j<$col; $j++)
        {
            ...
            if($row=$dsql->GetArray())
            {
                ...
                //处理同级栏目中，当前栏目的样式
                if( ($row['id'] == $refObj->TypeLink->TypeInfos['id'] || $row['id'] == $refObj->TypeLink->TypeInfos['reid'] || ($row['id'] == $topid && $type == 'top') ) && $currentstyle != '')
                {
                ...
                else
                {
                    ...
                    // 在新窗口打开外部链接
                    if($row['ispart']==2){
                        $likeType = str_replace("href",'target="_blank" href',$likeType);
                    }
                }
            }
            if($col>1) $likeType .= "</dd>\r\n";
