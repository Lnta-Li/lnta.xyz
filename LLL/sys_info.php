...
function ReWriteConfig()
{
    ...
    {
        if(preg_match("#[^0-9a-z_]#i", $row['varname']))
        {
            ShowMsg("变量名非法!", "-1");
            exit();
        }
        ...
        if($row['type']=='number')
        {
            ...
            fwrite($fp,"\${$row['varname']} = ".intval($row['value']).";\r\n");
        }
        else
        {
            fwrite($fp,"\${$row['varname']} = '".str_replace("",'', addslashes($row['value']))."';\r\n");
        }
    }
    fwrite($fp,"?".">");
    fclose($fp);
...
<?php
    ...
    {
        echo "<textarea name='edit___{$row['varname']}' row='4' id='edit___{$row['varname']}' class='textarea_info' style='width:98%;height:50px'>".addslashes($row['value'])."</textarea>";
    }else if($row['type']=='number')
    {
        echo "<input type='text' name='edit___{$row['varname']}' id='edit___{$row['varname']}' value='{$row['value']}' style='width:30%'>";
    }else
    {
        echo "<input type='text' name='edit___{$row['varname']}' id='edit___{$row['varname']}' value=\"".addslashes($row['value'])."\" style='width:80%'>";
    }
    ?>
</td>
