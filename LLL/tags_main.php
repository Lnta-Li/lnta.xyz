...
else if($action == 'update')
{
    ...
    $query = "UPDATE `#@__tagindex` SET `count`='$count', tag_alias='$tag_alias' WHERE id='$tid' ";
    ...
    while($row = $dsql->GetArray())
    {
        ...
        foreach($keyarr as $keyword)
        {
            ...
            if($keyword != '' && strlen($keyword)<251 )
            {
                ...
                {
                    $query = " INSERT INTO `#@__tagindex`(`tag`,`tag_alias`,`count`,`total`,`weekcc`,`monthcc`,`weekup`,`monthup`,`addtime`) VALUES('$keyword','$keyword','0','1','0','0','$timestamp','$timestamp','$timestamp');";
                    $dsql->ExecuteNoneQuery($query);
                    $tid = $dsql->GetLastID();
                }
