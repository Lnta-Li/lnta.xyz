...
$typename = dede_htmlspecialchars($typename);
...
else if($dopost=='savequick')
{
    ...
    if (empty($savetype))
    {
        ...
        foreach($_POST as $k=>$v)
        {
            ...
            $sql = str_replace('~topid~', GetTopid($reid), $sql);
            $sql = str_replace('~rank~', $rank, $sql);
            $sql = str_replace('~typename~', $toptypename, $sql);
            $sql = str_replace('~typedir~', $toptypedir, $sql);
            $dsql->ExecuteNoneQuery($sql);
...
else if($dopost=='save')
{
    ...
    $in_query = "INSERT INTO `#@__arctype`(reid,topid,sortrank,typename,typeenglishname,typebigimg,typesmallimg,typedir,isdefault,defaultname,issend,channeltype,
    ...
    VALUES('$reid','$topid','$sortrank','$typename','$typeenglishname','$typebigimg','$typesmallimg','$typedir','$isdefault','$defaultname','$issend','$channeltype',
